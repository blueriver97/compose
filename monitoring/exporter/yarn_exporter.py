...
class YarnRunningApplicationCollector(YarnCollector):
    ...
    def collect(self):
        ...
        running_apps_metric = YarnMetric('running_apps_info', YarnMetric.GAUGE,
                                         'Information about running applications in the YARN cluster',
                                         ['cluster', 'id', 'user', 'name', 'launch_time', 'elapsed_time', 'state',
                                          'final_status', 'master_node_id']).create_metric()

        for app_info in response.json()['apps']['app']:
            app_id = app_info['id']
            app_user = app_info['user']
            app_name = app_info['name']
            app_launch_time = str(app_info['launchTime'])
            app_elapsed_time = str(app_info['elapsedTime'])
            app_state = app_info['state']
            app_final_status = app_info['finalStatus']
            master_node_id = app_info['masterNodeId']
            running_apps_metric.add_metric(
                [self.cluster_name, app_id, app_user, app_name, app_launch_time, app_elapsed_time, app_state,
                 app_final_status, master_node_id], 1)

        yield running_apps_metric
...
if __name__ == "__main__":
    ...
    REGISTRY.register(YarnRunningApplicationCollector(args.yarn_url, args.cluster_name))
