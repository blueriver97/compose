...
def process_batch(batch_df: DataFrame, batch_id: int, spark: SparkSession, config: Settings) -> None:
    ...
    for table_identifier in config.TABLE_LIST:
        ...
        for row in distinct_schemas:
            ...
            # Avro 역직렬화 및 전처리
            transformed_df = filtered_df.filter(F.col("value_schema_id") == v_sid).withColumn(
                "data",
...
if __name__ == "__main__":
    ...
    query = (
        kafka_df
        .withColumn("key_schema_id", udf_byte2int(F.substring("key", 2, 4)))  # SCHEMA_ID = b'\x00\x00\x00\x08'
