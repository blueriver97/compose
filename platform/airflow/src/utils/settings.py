...
class Settings(BaseSettings):
    ...
    PARQUET_S3_ROOT_PATH: Optional[str] = Field(None, description="Root S3 path for Parquet files")

    # Note. Kafka Settings
    BOOTSTRAP_SERVERS: Optional[str] = Field(None, description="Kafka bootstrap servers")
    SCHEMA_REGISTRY: Optional[str] = Field(None, description="URL for the Schema Registry")
    TOPIC_PREFIX: Optional[str] = Field(None, description="Prefix for Kafka topics")
    METRIC_NAMESPACE: Optional[str] = Field(None, description="Namespace for metrics")
