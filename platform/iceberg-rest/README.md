...
- Credential Vending이나 Remote Signing 같은 고급 보안 기능을 통해 테이블 접근 권한을 세밀하게 제어할 수 있습니다.
- Credential Vending: 클라이언트(Spark)에게 마스터 키를 공유하지 않고, 특정 테이블에만 접근 가능한 임시 토큰(Token)을 발급하여 최소 권한 원칙(Least
  Privilege)을 준수합니다.
- Remote Signing: 클라이언트가 스토리지 자격 증명을 전혀 몰라도, 서버가 서명한 URL(Pre-signed URL)을 통해 안전하게 데이터를 읽고 쓸 수 있는 Zero
  Trust 환경을 구현합니다.
