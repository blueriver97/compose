...
Python 등 인터프리터 언어 빌드 시 `Profile Guided Optimization (PGO)` 및 `Link Time Optimization (LTO)` 옵션을 활성화하여 실행 속도를 10~20%
향상시킵니다.
...
로컬 LLM 구동을 위한 Ollama 설치 및 실행 스크립트를 제공합니다.

- 경로: `ollama/`
- 설치:
  ```bash
  sh ollama/install.sh
  sh ollama/run_ollama.sh
  ```
